...
// TestUsageCacheFillScopesLegacyDevinSourceUUIDs covers stored rows that
// never crossed a translation seam — e.g. an orphan copy predating the
// restamp, or a remote push written by an older binary. Bare Devin
// node/step ids are only unique per session, so the fill must emit the
// session-scoped identity the parser now produces: two sessions sharing
// bare uuid "2" extract as distinct source identities, under both local
// "devin:<raw>" and remote "host~devin:<raw>" session ids, while a
// non-Devin host-prefixed session keeps its bare uuid.
func TestUsageCacheFillScopesLegacyDevinSourceUUIDs(t *testing.T) {
	database := testDB(t)
	started := "2026-08-10T08:00:00Z"
	insertSession(t, database, "devin:sess-a", "project", func(s *Session) {
		s.Agent = "devin"
		s.StartedAt = &started
	})
	insertSession(t, database, "host~devin:sess-b", "project", func(s *Session) {
		s.Agent = "devin"
		s.Machine = "host"
		s.StartedAt = &started
	})
	insertSession(t, database, "host~other:sess-c", "project", func(s *Session) {
		s.Machine = "host"
		s.StartedAt = &started
	})
	...
	rows, err := cache.db.Query(`
		SELECT s.session_id, f.source_uuid
		FROM usage_facts f
		JOIN usage_cached_sessions s ON s.id = f.cached_session_id
		WHERE f.source = 'message'
		ORDER BY s.session_id`)
	require.NoError(t, err)
	defer rows.Close()
	for rows.Next() {
		var sessionID, sourceUUID string
		require.NoError(t, rows.Scan(&sessionID, &sourceUUID))
		got[sessionID] = sourceUUID
	}
	require.NoError(t, rows.Err())
	require.Len(t, got, 3)
	assert.Equal(t, "sess-a:2", got["devin:sess-a"],
		"bare Devin uuid must extract in session-scoped form")
	assert.Equal(t, "sess-b:2", got["host~devin:sess-b"],
		"host-prefixed Devin uuid must scope to the raw session id")
	assert.Equal(t, "2", got["host~other:sess-c"],
		"non-Devin host-prefixed uuid must pass through unchanged")
	assert.NotEqual(t, got["devin:sess-a"], got["host~devin:sess-b"],
		"shared bare uuids must not collapse into one dedup identity")
}
